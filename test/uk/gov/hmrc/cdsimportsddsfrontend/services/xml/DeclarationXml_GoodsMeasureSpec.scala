  ...
  "GoodsMeasure data" should {
    "be populated in the XML at item level" when {
      "the NetNetWeightMeasure is present in the data" in {
        val declaration = Declaration(
          commodity = Some(Commodity(description = None, classification = None, goodsMeasure = Some(GoodsMeasure(netNetWeightMeasure = Some("123"), tariffQuantity = Some("345"), grossMassMeasure = Some("678")))))
        )
    ...
    "be omitted from XML" when {
      "the no GoodsMeasure data exists" in {
        val declaration = Declaration(
          commodity = Some(Commodity(description = None, classification = None, goodsMeasure = None))
        )
