...
import uk.gov.hmrc.cdsimportsddsfrontend.services.{CustomsDeclarationsResponse, CustomsDeclarationsService, DeclarationServiceResponse, DeclarationStore}
  ...
  "A POST Request" should {
    "when XML is valid" should {
      "Submit to the declaration service and return a success response" in signedInScenario { user =>
        ...
        val customsDeclarationsServiceMockSetup: CustomsDeclarationsService => Unit = ds => when(ds.submit(any(), any[Elem] )(any())).thenReturn(Future.successful(DeclarationServiceResponse(<foo></foo>, CustomsDeclarationsResponse(200, Some("Good")))))
